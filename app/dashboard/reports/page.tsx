        ...
        <div className="bg-white/80 rounded-3xl p-6 sm:p-10 shadow-xl border border-navy/5 backdrop-blur-xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-end mb-8 border-b border-navy/10 pb-6">
              ...
              <Link href="/#lapor" className="btn-primary px-5 py-2.5 text-sm hidden sm:block">
                + Buat Laporan
              </Link>
            </div>
            ...
            {!reports || reports.length === 0 ? (
              ...
              <div className="space-y-4">
                {reports.map((report: any) => (
                  <div key={report.id} className="bg-white border border-navy/10 rounded-xl p-5 hover:shadow-lg hover:border-blue/30 transition-all duration-200">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <span className={`text-xs font-bold px-3 py-1 rounded-full border ${STATUS_COLORS[report.status] || STATUS_COLORS.pending}`}>
                        ...
                        <h3 className="font-bold text-navy text-lg line-clamp-1">{report.title}</h3>
                        <p className="text-sm text-navy/50 mt-1 capitalize">
                          Kategori: {Array.isArray(report.categories) ? report.categories[0]?.name : report.categories?.name || "Lainnya"}
                        </p>
                      </div>
